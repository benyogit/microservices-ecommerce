...
import { validateBody, validateQuery } from '../../utils/http/validate';
import { container } from '../../utils/di/container';
import { TYPES } from '../../utils/di/types';
import { ProductController } from './product.controller';
import { addProductImageSchema, createProductSchema, listProductsQuerySchema } from './product.schema';

export const productRouter = Router();

const controller = container.get<ProductController>(TYPES.ProductController);

productRouter.get('/', validateQuery(listProductsQuerySchema), asyncHandler(controller.list));
